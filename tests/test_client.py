...
from pepkio_molarity_solution_calculator.config import TOOL_ID
from pepkio_molarity_solution_calculator.exceptions import PepkioAPIError
from pepkio_molarity_solution_calculator.models import RunResult
...
def test_get_manifest_and_cache(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert m1["tool_id"] == "molarity-solution-calculator"
        assert m1 is m2
    assert len(calls) == 1
...
def test_list_examples_and_get_example_input(mock_manifest):
    ...
    with _make_client(handler) as client:
        ...
        assert inp["mode"] == "powder"
        assert inp["target_concentration_unit"] == "mM"
...
def test_run_sends_correct_request(mock_manifest, mock_run_response):
    ...
    with _make_client(handler) as client:
        result = client.run(client.get_example_input("powder_cuso4"), label="pytest")

    assert captured["path"] == f"/api/tools/v1/tools/{TOOL_ID}/run"
    assert captured["auth"] == "Bearer test-key"
    assert captured["body"]["input"]["mode"] == "powder"
    assert captured["body"]["options"]["label"] == "pytest"
    assert result.status == "completed"
    assert result.result["mode"] == "powder"
    assert result.result["result"]["mass_g"] == 0.081
